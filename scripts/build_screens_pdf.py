...
def capture_flows(tmp):
    ...
    with sync_playwright() as pw:
        ...
        sel("tax", "npd"); rng(70000)
        shot("calc-70", [".controls", "#calc"])
        rng(50000); sel("scheme", "fixnom")
        shot("calc-fixnom", [".controls", "#calc"])
        sel("scheme", "fixnet")
        shot("calc-fixnet", [".controls", "#calc"])
        sel("scheme", "share"); rng(35000); sel("route", "2")
...
FLOWS_PAGES = [
    ("Сценарий и расчёт", "Калькулятор сделки",
     "Задаются все цены, доля партнёра и конструкция расчёта. Ниже — что получает семья, партнёр и платформа при этих значениях и сравнение с прежней формулой."),
    ...
    ("Варианты расчёта", "Партнёр — физическое лицо",
     "Взносы 30 процентов сверх начисления ложатся на платформу, поэтому её результат ниже, чем на других режимах."),
    ("Варианты расчёта", "Цена 70 000, верхней границы нет",
     "Граница снята 13 сентября. При доле от цены платформа растёт вместе с партнёром, поэтому предохранитель не нужен."),
    ("Варианты расчёта", "Прежняя конструкция: фикс номиналом",
     "Платформа удерживает фиксированную сумму, а налог и эквайринг считаются со всей цены: её результат падает с ростом цены."),
    ("Варианты расчёта", "Фикс чистыми, расходы на партнёре",
     "Платформа получает свою сумму при любой цене, оборотные расходы вычитаются из начисления партнёра. Формулировку в оферте согласовать с юристом."),
